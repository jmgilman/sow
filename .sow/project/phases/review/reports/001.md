...
After initial review, the following issues were discovered and fixed:

1. **Test Organization**: Renamed `unified_commands/` to flat structure by command group (artifacts/, phase/, project/, tasks/, integration/)
2. **Critical Bug**: Fixed nil pointer panic in `sow project new` - added directory creation for `.sow/project/` before Save()
3. **Test Cleanup**: Removed worktree tests (testing old commands, out of scope)
4. **Test Fixes**: Updated phase and task tests to use correct project initialization

**All tests now passing**: 21/21 integration tests pass ✅
