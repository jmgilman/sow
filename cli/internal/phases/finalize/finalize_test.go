...
func TestEntryState(t *testing.T) {
	...
	if phase.EntryState() != phases.FinalizeDocumentation {
...
func TestAddToMachine(t *testing.T) {
	sm := stateless.NewStateMachine(phases.FinalizeDocumentation)
	phase := New(nil, ProjectInfo{})

	phase.AddToMachine(sm, phases.NoProject)

	canFire, _ := sm.CanFire(phases.EventDocumentationDone)
	if !canFire {
...
func TestFullTransitionFlow_ThreeStages(t *testing.T) {
	...
	sm.Fire(phases.EventProjectDelete)
	currentState = sm.MustState().(phases.State)
	if currentState != phases.NoProject {
		t.Errorf("Expected state to be NoProject, got %s", currentState)
	}
}

func TestFullTransitionFlow_DeleteBlocked(t *testing.T) {
	sm := stateless.NewStateMachine(phases.FinalizeDelete)
