...
// TestTransitions_ActiveToFinalizing_OnExitMarksDesignPhaseComplete tests that
// the OnExit action marks the design phase as completed.
func TestTransitions_ActiveToFinalizing_OnExitMarksDesignPhaseComplete(t *testing.T) {
	config := NewDesignProjectConfig()
	...
	// Add a completed task to satisfy guard
	phase := proj.Phases["design"]
	phase.Tasks = []projschema.TaskState{
		{
			Id:     "task-1",
	...
	// Verify design phase is marked as completed
	designPhase := proj.Phases["design"]
	assert.Equal(t, "completed", designPhase.Status, "design phase should be completed")
	assert.False(t, designPhase.Completed_at.IsZero(), "design phase should have Completed_at set")
}

// TestTransitions_ActiveToFinalizing_OnEntryEnablesFinalizationPhase tests that
// the OnEntry action enables and activates the finalization phase.
func TestTransitions_ActiveToFinalizing_OnEntryEnablesFinalizationPhase(t *testing.T) {
	config := NewDesignProjectConfig()
	...
	// Verify finalization phase is enabled and in progress
	finalizationPhase := proj.Phases["finalization"]
	assert.True(t, finalizationPhase.Enabled, "finalization phase should be enabled")
	assert.Equal(t, "in_progress", finalizationPhase.Status, "finalization phase should be in_progress")
	assert.False(t, finalizationPhase.Started_at.IsZero(), "finalization phase should have Started_at set")
}
...
// TestTransitions_FinalizingToCompleted_OnEntryMarksFinalizationPhaseComplete tests that
// the OnEntry action marks the finalization phase as completed.
func TestTransitions_FinalizingToCompleted_OnEntryMarksFinalizationPhaseComplete(t *testing.T) {
	config := NewDesignProjectConfig()
	...
	// Fire transition to Completed
	err = machine.Fire(sdkstate.Event(EventCompleteFinalization))
	require.NoError(t, err)

	// Verify finalization phase is marked as completed
	finalizationPhase = proj.Phases["finalization"]
	assert.Equal(t, "completed", finalizationPhase.Status, "finalization phase should be completed")
	assert.False(t, finalizationPhase.Completed_at.IsZero(), "finalization phase should have Completed_at set")
}
