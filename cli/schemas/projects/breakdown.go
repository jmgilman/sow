...
type BreakdownProjectState struct {
	...
	Phases struct {
		// Phase 1: Breakdown
		Breakdown BreakdownPhase `json:"breakdown"`

		// Phase 2: Finalization
		Finalization BreakdownFinalizationPhase `json:"finalization"`
	} `json:"phases"`
}

// BreakdownPhase is the breakdown phase for breakdown projects.
type BreakdownPhase struct {
	p.Phase

	// Note: Breakdown phase uses common Phase fields plus status constraint
}

// BreakdownFinalizationPhase is the finalization phase for breakdown projects.
type BreakdownFinalizationPhase struct {
	p.Phase
